...
**If you say no.** These features stay off. Borssaint sees the moment you grant the permission and brings them to life with no relaunch.
...
**If you say no.** Borssaint runs without a peep, and the same information is still right there in the panel and in Settings.
...
**Optional.** Yes. There is no pop up for Full Disk Access. You add Borssaint in System Settings, under Privacy and Security, Full Disk Access, and Borssaint opens that pane for you when the feature calls for it.

## Administrator, one time and optional

**Why it comes up.** Keeping the Mac awake with the lid shut relies on `pmset disablesleep`, which needs administrator rights. So it does not have to ask for your password every time you flip closed lid mode, Borssaint can install a tightly scoped `sudoers` rule that allows only that single command.
...
**Optional.** Yes, and it is a one time choice. The rule is limited to `pmset disablesleep` and nothing else, and it goes away on its own when you uninstall Borssaint or reset things from Settings under Advanced.
